...
const SecurityQ = () => {
    ...
    return (
        <div className={styles.sec_contain}>
            ...
            <button className={styles.submit} onClick={handleSubmit}>Submit Answer</button>
            {submit &&(
                <div className={styles.ans_rev}>
                    <h2>Your answers:</h2>
                    <ul>
                        {ans.map((a,i)=>(
                            <li key={i}><strong>{questions[i]}</strong>: {a || "No answer provided"}</li>
                        ))}
                    </ul>
                </div>
            )}
